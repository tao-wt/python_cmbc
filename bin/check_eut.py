...
def helpFunc(a,b,c,d):
	...
	print "-s or --super yes or not check supervise,default:False. example:\033[1;33;40m python %s -a -s\033[0m" %sys.argv[0]
	sys.exit(3)
...
#def check_PingHost(ip):
#    hosts = []
#    for i in ip:
#      if subprocess.call('ping -c2 -W 1 -i 0.2 %s > /dev/null' % i, shell=True) == 0 or subprocess.call('ping -c2 -W 1 -i 0.2 %s > /dev/null' % i, shell=True) == 0:
#        #print '%s ping is: \033[32;1m OK \033[0m' % i
#        i = ''.join(i)
#        hosts.append(i)
#      else:
#        print '------\033[1;33;40m%s\033[0m \033[31;1mping not connect! \033[0m------' %i
#    return hosts
...
def checkmoduleinfo_super(moduleinfo):
	host = moduleinfo[0]
	moduleport = moduleinfo[1]
	path = moduleinfo[2]
	modulerunname = path.split('/')[-1]
	...
	ps = "ps -ef|grep %s.jar|grep -v supervise|grep -v grep > /dev/null && echo '1' || echo '0'" %(modulerunname)
	result_ps = p.check_cmd(ps).strip()
	super_ps = "ps -ef|grep supervise|grep %s|grep -v grep > /dev/null && echo '1' || echo '0'" %(modulerunname)
	result_super_ps = p.check_cmd(super_ps).strip()
	portnumber = len(moduleport.split(";"))
	if portnumber == 2:
		count = 0
		for i in moduleport.split(";"):
			...
			result_netstat_count = int(p.check_cmd(cmd_netstat).strip())
			count = count + result_netstat_count
		if count == portnumber and result_ps == '1' and result_super_ps == '1':
			print "%s %s \033[1;32;40mprogram and supervise start OK\033[0m" %(host,modulerunname)
		elif count == portnumber and result_ps == '1' and result_super_ps == '0':
			print "%s %s \033[1;32;40mprogram start OK!\033[0m \033[1;31;40mbut supervise not OK\033[0m" %(host,modulerunname)
		elif count == portnumber and result_ps == '0' and result_super_ps == '0':
			print "%s %s \033[1;31;40mprogram and supervise not start!\033[0m" %(host,modulerunname)
		elif count != portnumber and result_super_ps == '1':
			print "%s %s \033[1;31;40mprogram no listen!\033[0m \033[1;32;40mbut supervise start OK!\033[0m" %(host,modulerunname)
		else:
			print "%s %s \033[1;31;40mprogram and supervise not start!\033[0m" %(host,modulerunname)
	elif portnumber == 1:
		cmd_netstat = "netstat -an|grep LISTEN|grep tcp|grep -w '%s' > /dev/null && echo '1' || echo '0'" %(moduleport)
		result_netstat = p.check_cmd(cmd_netstat).strip()
		if result_netstat == '1' and result_ps == '1' and result_super_ps == '1':
			print "%s %s \033[1;32;40mprogram and supervise start OK\033[0m" %(host,modulerunname)
		elif result_netstat == '1' and result_ps == '1' and result_super_ps == '0':
			print "%s %s \033[1;32;40mprogram start OK!\033[0m \033[1;31;40mbut supervise not OK\033[0m" %(host,modulerunname)
		elif result_netstat == '1' and result_ps == '0' and result_super_ps == '0':
			print "%s %s \033[1;31;40mprogram and supervise not start!\033[0m" %(host,modulerunname)
		elif result_netstat == '0' and result_super_ps == '1':
			print "%s %s \033[1;31;40mprogram no listen!\033[0m \033[1;32;40mbut supervise start OK!\033[0m" %(host,modulerunname)
		else:
			print "%s %s \033[1;31;40mprogram and supervise not start!\033[0m" %(host,modulerunname)
...
def multipoolcess_super(moduleinfo):
	pool = multiprocessing.Pool(processes=8)
	modlen =  len(moduleinfo)
	count = modlen/2
	linfo = modlen%2
	...
	if mid == "head":
		i = 0
		while i < count:
			pool.apply_async(checkmoduleinfo_super,(moduleinfo[i],))
			i = i+1
	elif mid == "tail":
		i = count
		while i < modlen:
		...
		for i in moduleinfo:
			pool.apply_async(checkmoduleinfo_super,(i,))
	pool.close()
	pool.join()
...
#ip_check_supervise
elif ip != [''] and mod == [''] and super == True:
	for i in ip:
		tmplist.append(t.findModByIP(i))
	moduleappend()
	multipoolcess_super(moduleinfo)
#mod_check
	...
	multipoolcess(moduleinfo)
#mod_check_supervise
elif mod != [''] and ip == [''] and super == True:
	for m in mod:
		tmplist.append(t.findModByModName(m))
	moduleappend()
	multipoolcess_super(moduleinfo)
