...
#判断输入的ip及模块名称是否合法，是否存在在配置文件中。
	if mod != [''] and ip != [''] and (cfilename=="" and runcmd==""):
		tmplist = []
		infolist = []
		for i in mod:
			for n in ip:
...
#对模块进行批量部署，更新模块也是这一判断
	elif mod != [''] and allip == True and (cfilename=="" and runcmd==""):
		orderStr = ""
		if order != ['']:
			if '' in order:
				order.remove('')
		...
		if mod != ['']:
			modStr = " where "
			for n in mod:
				modStr = modStr + "modulename='%s' or "%n
		...
		if slen == -1:
			...
			if ip != ['']:
				...
				oaIp = t.FindAllIpPathD(modStr,orderStr)
				for m in oaip:
					for i in m:
						aip.append(i)
			for i in aIp:
				Cp = ParamikoClass(i,port,user,passwd)
