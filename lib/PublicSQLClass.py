...
class SQLClass(object):
	...
	def findIPInfo(self,modname):
		sql = "select ip,publicname,publicpath,cpname from public where publicname = '%s'" %modname
		return self.s.queryAll(sql)
	...
	def findModByIP(self,host):
		sql = "select ip,publicname,startcmd,port from PUBLIC where ip = '%s';" %host
		return self.s.queryAll(sql)
	...
	def findModByModName(self,publicname):
		sql = "select ip,publicname,startcmd,port from public where publicname = '%s';" %publicname
		return self.s.queryAll(sql)

	def findModByModAndIP(self,host,publicname):
		sql = "select ip,publicname,startcmd,port from public where ip = '%s' and publicname = '%s';" %(host,publicname)
		return self.s.queryAll(sql)
	...
	def findIpOnlyMod(self,mod):
		sql = "select ip,startcmd from public where publicname = '%s';" %mod
		return self.s.queryAll(sql)
