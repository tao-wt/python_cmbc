...
class SQLClass(object):
	...
	def findModByModName(self,modname):
		sql = "select ip,port,path from module where modulename = '%s';" %modname
		return self.s.queryAll(sql)

	def findModByIP(self,host):
		sql = "select ip,port,path from module where ip = '%s';" %host
		return self.s.queryAll(sql)

	def findModByModAndIP(self,host,modname):
		sql = "select ip,port,path from module where ip = '%s' and modulename = '%s';" %(host,modname)
		return self.s.queryAll(sql)
	#sainter_add_end
